...
/// <summary>
/// 针床清单（SYST）维护页：左侧 SYST 列表（新增/删除），右侧选中清单的元信息 + 号位 + 测试项编辑，
...
public partial class ManifestMaintenanceViewModel : ObservableObject
{
    ...
    /// <summary>
    /// 左侧 SYST 列表（一级设备族分组，二级板子；支持搜索与整体展开/折叠）。
    /// </summary>
    ...
    /// <summary>
    /// 当前清单下可用的测试项 Kind（含通用处理器：DeviceFamily 为空）。
    ...
    private void DeleteManifest()
    {
        ...
        if (!AppDialog.ConfirmDanger("删除 SYST", $"确认删除清单【{item.Device} · {item.Board}】(Key={item.Key})？\n将删除对应 JSON 文件，不可恢复。"))
        {
            return;
        }
        ...
        {
            CatalogChanged = true;
            _notify.Notify($"已删除 SYST：{item.Board}");
            Current = null;
            RefreshList(null);
        }
        else
    ...
    private void SaveManifest()
    {
        ...
        try
        {
            ...
            m.OriginalKey = m.Key;
            CatalogChanged = true;
            _notify.Notify($"已保存 SYST：{m.BoardName}");
            RefreshList(m.Key);
