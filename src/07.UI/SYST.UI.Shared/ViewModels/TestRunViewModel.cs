...
using SYST.Automation;
using SYST.Core.Abstractions;
using SYST.Core.Engine;
using SYST.Devices;
using SYST.Devices.Comm;
using SYST.Infrastructure.Auth;
using SYST.Infrastructure.Configuration;
using SYST.Infrastructure.Data;
using SYST.Infrastructure.Notifications;
using SYST.UI.Shared.BoardExtras;
using SYST.UI.Shared.Services;
using SYST.UI.Shared.Views;
using SYST.UI.Shared.Views.Dialogs;

namespace SYST.UI.Shared.ViewModels;
...
public partial class TestRunViewModel : ObservableObject
{
    ...
    /// <summary>
    /// 当前实时曲线跟踪的测试项（步骤开始运行时锁定；步骤结束后保留显示直到下一个步骤开始，
    /// 便于操作员查看该项最终曲线）。为 null 时曲线面板隐藏。
    /// </summary>
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(HasLiveCurve))]
    private StepCellViewModel? _liveStep;

    /// <summary>
    /// 是否有实时曲线数据（当前跟踪步骤至少采集到 2 个采样点）。
    /// 无数据的测试项（SN 写入/设备类型写入等）曲线面板保持折叠/隐藏；
    /// 通过 foreach/while 循环采集多点时才自动展示。
    /// </summary>
    public bool HasLiveCurve => LiveStep?.HasCurveData == true;

    /// <summary>
    /// 曲线面板是否展开（默认展开；操作员可点击头部折叠让号位卡片占满高度）。
    /// </summary>
    [ObservableProperty] private bool _isCurvePanelOpen = true;

    /// <summary>
    /// 切换曲线面板展开/折叠。
    /// </summary>
    [RelayCommand]
    private void ToggleCurvePanel()
    {
        IsCurvePanelOpen = !IsCurvePanelOpen;
    }
    ...
    private async Task StartStop()
    {
        ...
        // 开始前人工确认：测试准备（上下料/接线确认，对应旧平台 PreparationTask/TestReady 弹窗）
        if (!AppDialog.Confirm("测试前准备确认",
                "1、接入4根电测线，2个外接航插线，1根电源线。\r\n" +
                "2、接入USB通讯线，网线，U盘。\r\n" +
                "3、拧上设备侧边的堵头。\r\n\r\n" +
                "完成后点击【确认】开始测试；点击【取消】停止。"))
        {
            _notify.Notify("已取消：测试前准备未确认");
            return;
        }

        // 开始前自动重置数据 + 生成本次 SN（保留操作员手动输入的号位 SN，其余自动生成）
        _counters.Reset();
        var manualSns = Positions.Where(p => p.IsSelected && !string.IsNullOrWhiteSpace(p.SerialNumber))
                                 .ToDictionary(p => p.Index, p => p.SerialNumber!);
        foreach (var p in Positions)
        {
            p.Reset();
        ...
        foreach (var p in Positions.Where(p => manualSns.ContainsKey(p.Index)))
        {
            p.SerialNumber = manualSns[p.Index];
        }
    ...
    private async Task SingleTest(StepCellViewModel? cell)
    {
        ...
        // 带上号位 SN（操作员手动输入），供 SN 写入等依赖 ctx.SerialNumber 的测试项使用；未输入则不传
        var singlePos = Positions.FirstOrDefault(p => p.Index == cell.PositionIndex);
        var singleSn = singlePos?.SerialNumber;
        IReadOnlyDictionary<int, string>? singleSerials = null;
        if (!string.IsNullOrWhiteSpace(singleSn))
        {
            singleSerials = new Dictionary<int, string> { [cell.PositionIndex] = singleSn };
        }

        var options = new RunOptions
        {
            Positions = [cell.PositionIndex],
            StepKeys = [cell.Key],
            SerialNumbers = singleSerials,
        };
    ...
    private void StopLingeringSteps()
    {
        ...
        TaskbarProgressValue = 0;
        TaskbarProgressState = TaskbarItemProgressState.None;
        // 测试停止后保留 LiveStep 曲线（便于查看最后一项的结果曲线），不主动清空；
        // 下一次开始测试的 StartNewCycle 会清空。
    }

    /// <summary>
    /// 人工确认请求（整机模板 ManualStep）：投递到 UI 线程弹确认框（非模态），
    /// 操作员点 通过/不合格 后回传引擎，该号位继续；其他号位不受阻塞。
    /// </summary>
    /// <param name="sender">事件源。</param>
    /// <param name="e">确认请求参数。</param>
    private void OnManualConfirmRequested(object? sender, ManualConfirmRequestedEventArgs e)
    {
        _dispatcher.BeginInvoke(() =>
        {
            var dlg = new ManualConfirmDialog(e);
    ...
    /// 步骤进入运行态时锁定为 <see cref="LiveStep"/>（曲线面板跟随其实时采样）。
    /// </summary>
    /// <param name="sender">事件源。</param>
    ...
    private void OnStepChanged(object? sender, StepProgressEventArgs e)
    {
        _dispatcher.Invoke(() =>
        {
            ...
            if (pos is not null)
            {
                // 步骤开始运行 → 锁定为实时曲线跟踪项（多号位并行时取最后开始的一项）
                if (e.Phase == StepPhase.Running)
                {
                    var cell = pos.Steps.FirstOrDefault(c => string.Equals(c.Key, e.Step.Key, StringComparison.OrdinalIgnoreCase));
                    if (cell is not null)
                    {
                        LiveStep = cell;
                    }
    ...
    private void OnSample(object? sender, SampleEventArgs e)
    {
        _dispatcher.BeginInvoke(() =>
        {
            var pos = Positions.FirstOrDefault(p => p.Index == e.PositionIndex);
            pos?.AddSample(e);
            // 采样点到达后刷新曲线面板显示状态（首个数据点到达时才展开）
            if (LiveStep is not null && string.Equals(LiveStep.Key, e.StepKey, StringComparison.OrdinalIgnoreCase))
            {
...
public partial class StepCellViewModel(int positionIndex, int order, string key, string name, string description = "", string conditionsText = "") : ObservableObject
{
    ...
    /// </summary>
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(HasCurveData))]
    private ProcessDataSeries? _processData;

    /// <summary>
    /// 是否有曲线数据（至少 2 个采样点才有意义）。无数据（如 SN 写入/设备类型写入等
    /// 无采集的测试项）时实时曲线面板保持折叠/隐藏。
    /// </summary>
    public bool HasCurveData => _sampleTime.Count >= 2;
    ...
    /// </summary>
    public string LatestLog => Details.Count > 0 ? $"{Details[^1].Time} {Details[^1].Message}" : "";

    /// <summary>
    /// 完整过程信息（时间 + 消息逐行拼接，悬停测试项行查看；无过程时为空）。
    /// </summary>
