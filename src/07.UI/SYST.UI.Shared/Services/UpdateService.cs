...
namespace SYST.UI.Shared.Services;
...
public sealed class UpdateService
{
    ...
    /// </summary>
    private const string UpdaterExe = "SYST.Updater.exe";

    /// <summary>
    /// 主程序文件名（升级器换目录后要重启的目标）。
    /// </summary>
    private const string AppExe = "SYST.App.exe";
    ...
    private static string StageUpdater()
    {
        ...
        if (!File.Exists(src))
        {
            throw new FileNotFoundException("缺少升级器 SYST.Updater.exe（发布包应包含，请重新发布）。", src);
        }

        var dst = Path.Combine(Path.GetTempPath(), $"SYST.Updater_{Environment.TickCount64}.exe");
