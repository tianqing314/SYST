...
public sealed class TestPaModuleConST811AHandler : IStepHandler
{
    ...
    public async Task<StepResult> ExecuteAsync(ITestContext ctx, CancellationToken ct = default)
    {
        ...
        // 搜索并获取 PA 变送器列表失败时，确认后重新搜索（原 goto tryagain → RetryHelper 重构）
...
public sealed class TestHartConST811AHandler : IStepHandler
{
    ...
    public async Task<StepResult> ExecuteAsync(ITestContext ctx, CancellationToken ct = default)
    {
        ...
        var prevPass1 = pass;  // G8: 记录本重试段之前的整体结果
        int i = 0;  // 搜索 Hart 的重试计数（旧 `int i = 0`）
        while (true) {  // G8: 原 goto 标签 tryagain → while(true) 重试循环
            pass = true;  // G8: 每次重试重置本段结果
...
public sealed class TestMeterStateConST811AHandler : IStepHandler
{
    ...
    public async Task<StepResult> ExecuteAsync(ITestContext ctx, CancellationToken ct = default)
    {
        ...
        //断开适配器充电（原 goto tryagain → RetryHelper 重构）：读 27V 状态并断开充电，失败重试最多 3 次
        var retryOk = await RetryHelper.RetryAsync(async attempt =>
        {
            pass = true;  // 每次重试重置本段结果
        ...
        if (!retryOk) { op.Report("工装断开适配器充电失败，重测后没有改善，请将工装重新上电后重试", RealtimeLevel.Error); pass = false; }
        ...
        //读取主板自检状态（旧声明 `CheckState MainBoardCheckStata = ...` 类型未迁移，语义由 QueryBooleanAsync 承载）
        ...
        //读取供电方式（旧声明 `PowerType BatteryCheckStata = ...` 类型未迁移，语义由 QueryBooleanAsync 承载）
        ...
        // 27V 供电恢复（旧脚本以 GZP21 IsOpen 守卫 Set27VState Close/Open）
        await op.Gzp21.SetOutputAsync("27V", false, ct);
...
public sealed class ElectricalMeasurementAndOutputFunctionTestConST811AHandler : IStepHandler
{
    ...
    public async Task<StepResult> ExecuteAsync(ITestContext ctx, CancellationToken ct = default)
    {
        var op = new ConST811AOps(ctx, ct);
        var pass = true;
        //电测板测量/输出功能测试（原 goto tryagain → RetryHelper 重构）：切换档位并读数，测量失败可确认重试最多 3 次
        var retryOk = await RetryHelper.RetryAsync(async attempt =>
        {
            pass = true;  // 每次重试重置本段结果
            ...
            //读取电测量值
            if (!(await op.Dut.QueryBooleanAsync("GetCurrentElectricMeasure", null, ct))) { op.Report("读取测量值失败", RealtimeLevel.Error); pass = false; }
        ...
        //读取电测量值
        if (!(await op.Dut.QueryBooleanAsync("GetCurrentElectricMeasure", null, ct))) { op.Report("读取测量值失败", RealtimeLevel.Error); pass = false; }
        op.Report(pass ? "10mA 电流输出功能测试通过" : "10mA 电流输出功能测试不通过", pass ? RealtimeLevel.Info : RealtimeLevel.Warn);
        ...
        //读取电测量值
        if (!(await op.Dut.QueryBooleanAsync("GetCurrentElectricMeasure", null, ct))) { op.Report("读取测量值失败", RealtimeLevel.Error); pass = false; }
        op.Report(pass ? "断开24V,连接24V时输出10mA电流 测试通过" : "断开24V,连接24V时输出10mA电流 测试不通过", pass ? RealtimeLevel.Info : RealtimeLevel.Warn);
        ...
        //读取电测量值
        if (!(await op.Dut.QueryBooleanAsync("GetCurrentElectricMeasure", null, ct))) { op.Report("读取测量值失败", RealtimeLevel.Error); pass = false; }
        op.Report(pass ? "连接24V,断开24V时输出10mA电流 测试通过" : "连接24V,断开24V时输出10mA电流 测试不通过", pass ? RealtimeLevel.Info : RealtimeLevel.Warn);
...
public sealed class TestSwitchConST811AHandler : IStepHandler
{
    ...
    public async Task<StepResult> ExecuteAsync(ITestContext ctx, CancellationToken ct = default)
    {
        ...
        if (!(await op.Dut.QueryBooleanAsync("SetEleChannelItem_SW_Normal", null, ct))) { op.Report("打开机械开关失败", RealtimeLevel.Error); pass = false; }
        //机械(或NPN)开关测试--分开（原 goto trySW → RetryHelper 重构）：读测量并切开关，判定失败可确认重试最多 3 次
        ...
        op.Report(retryOk1 ? $"机械开关分开: {"√"}" : $"机械开关分开: {"机械(或NPN)开关测试未通过"}", retryOk1 ? RealtimeLevel.Info : RealtimeLevel.Warn);
        ...
        if (!(await op.Dut.QueryBooleanAsync("GetCurrentElectricMeasure", null, ct))) { op.Report("读取当前电测测量值失败", RealtimeLevel.Error); pass = false; }
        if (!(await op.Dut.QueryBooleanAsync("SetEleChannelItem_SW_Normal", null, ct))) { op.Report("打开机械开关失败", RealtimeLevel.Error); pass = false; }
        //机械(或NPN)开关测试--短接（原 goto trySW → RetryHelper 重构）：读测量并切开关，判定失败可确认重试最多 3 次
        ...
        op.Report(retryOk2 ? $"结果1: {"√"}" : $"结果1: {"机械(或NPN)开关测试未通过"}", retryOk2 ? RealtimeLevel.Info : RealtimeLevel.Warn);
        ...
        //PNP开关测试--闭合（原 goto trySW → RetryHelper 重构）：读测量并切开关，判定失败可确认重试最多 3 次
        ...
        op.Report(retryOk3 ? $"结果2: {"√"}" : $"结果2: {"PNP开关未通过"}", retryOk3 ? RealtimeLevel.Info : RealtimeLevel.Warn);
        ...
        //PNP开关测试--断开（原 goto trySW → RetryHelper 重构）：读测量并切开关，判定失败可确认重试最多 3 次
        ...
        op.Report(retryOk4 ? $"结果3: {"√"}" : $"结果3: {"PNP开关测试未通过"}", retryOk4 ? RealtimeLevel.Info : RealtimeLevel.Warn);
...
public sealed class RTCTimeTestConST811AHandler : IStepHandler
{
    ...
    public async Task<StepResult> ExecuteAsync(ITestContext ctx, CancellationToken ct = default)
    {
        ...
        //回读
        if (!(await op.Dut.QueryBooleanAsync("GetDevSysDate", null, ct))) { op.Report("GetDevSysDate 调用失败", RealtimeLevel.Error); pass = false; }
        op.Report($"电脑时间:{DateTime.Now}", RealtimeLevel.Info);
...
public sealed class TestOverallWIFIConST811AHandler : IStepHandler
{
    ...
    public async Task<StepResult> ExecuteAsync(ITestContext ctx, CancellationToken ct = default)
    {
        ...
        //if (!item.GetDevice("P21").OpenBlueTooth())
        //{
        //    result.AddTestErrMsgs(new ErrMsg(30003, "指令通讯异常，打开蓝牙失败"));
        //    return result;
        //}
        //msg12.Content = "√";
...
public sealed class TestCalibrationSensorConST811AHandler : IStepHandler
{
    ...
    public async Task<StepResult> ExecuteAsync(ITestContext ctx, CancellationToken ct = default)
    {
        ...
        //进气传感器校准（原 goto tryagain → RetryHelper 重构）：校准失败可确认重试最多 3 次
        var retryOk = await RetryHelper.RetryAsync(async attempt =>
        {
            pass = true;  // 每次重试重置本段结果
        ...
        //校准完成（原 isFinished = true 轮询控制流，循环已扁平化）
        await op.Dut.CommandAsync("StopCalibrationSensor", null, ct);
        //校准失败,校准过程异常
        await op.Dut.CommandAsync("StopCalibrationSensor", null, ct);
        await Task.Delay(1000, ct);
            return pass;
        }, _ => ctx.ConfirmAsync("校准失败,校准过程异常，点击【确认】重新校准，否则测试失败。", ct), 3, ct);
        if (!retryOk) { op.Report("校准失败,校准过程异常", RealtimeLevel.Error); pass = false; }
