...
using SYST.Devices;
using SYST.Infrastructure;
using SYST.Infrastructure.Auth;
using SYST.Jigs;
using SYST.TestSteps;
...
public partial class App : Application
{
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        // 现场诊断：设 SYST_BINDING_TRACE=1 时把 WPF 数据绑定错误落到 logs/binding-trace.log
        // （.NET 8 不再从 app.config 读 <system.diagnostics>，只能代码注册；未设环境变量时零开销）。
        if (Environment.GetEnvironmentVariable("SYST_BINDING_TRACE") == "1")
        {
    ...
    private void Bootstrap()
    {
        ...
        _host = Host.CreateDefaultBuilder()
            ...
            .ConfigureServices((ctx, services) =>
            {
                ...
                var hw = new SYST.Devices.HardwareOptions
                {
                ...
                if (hw.UseReal || Environment.GetEnvironmentVariable("SYST_REAL_HARDWARE") == "1")
                {
                    services.AddSingleton<SYST.Devices.Abstractions.IBleScanner, SYST.Devices.BleWin.Win10BleScanner>();
                }
                ...
                services.AddSingleton<SYST.UI.Shared.Services.UpdateService>();
        ...
        SYST.TestSteps.ProcessDataSimulator.StreamIntervalMs = 60;

        // 登录窗关闭到主窗显示之间会出现"零窗口"瞬间；默认 OnLastWindowClose 会在此刻退出整个程序。
        // 故先显式关闭、待主窗显示后再切回 OnMainWindowClose。
        ShutdownMode = ShutdownMode.OnExplicitShutdown;

        // 开发/自动化验证用：设 SYST_SKIP_LOGIN=1 可跳过登录直达主界面。
        var skipLogin = Environment.GetEnvironmentVariable("SYST_SKIP_LOGIN") == "1";
        string userName = "dev";
        ...
        // 否则旧客户端会往产线写坏数据。CI 冒烟（SYST_AUTORUN_TASK）不联网，跳过。
        var update = _host.Services.GetRequiredService<UpdateService>();
        update.Operator = userName;
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("SYST_AUTORUN_TASK"))
        ...
        // CI/自动化冒烟：SYST_AUTORUN_TASK=<板Key> 时自动打开并运行该板，跑完可选自动退出。
        var autorunKey = Environment.GetEnvironmentVariable("SYST_AUTORUN_TASK");
    ...
    private async Task AutoRunAsync(MainViewModel mainVm, string boardKey)
    {
        ...
        if (Environment.GetEnvironmentVariable("SYST_EXIT_AFTER") == "1")
        {
            Shutdown();
        }
    }
