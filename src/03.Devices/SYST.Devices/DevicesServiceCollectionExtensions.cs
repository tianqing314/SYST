...
using SYST.Devices.StandardBox;

namespace SYST.Devices;
...
{
    /// <summary>
    /// 是否走真机驱动（Xmas11）。也可用环境变量 SYST_REAL_HARDWARE=1 覆盖。
    /// </summary>
...
public static class DevicesServiceCollectionExtensions
{
    ...
    public static IServiceCollection AddPcbaDevices(this IServiceCollection services, HardwareOptions? hw = null)
    {
        ...
        // 真机开关：配置 Pcba:Hardware:UseReal 或环境变量 SYST_REAL_HARDWARE=1
        var useReal = (hw?.UseReal ?? false) || Environment.GetEnvironmentVariable("SYST_REAL_HARDWARE") == "1";
