...
public sealed class WmiSerialScanner : IDeviceScanner
{
    ...
    /// <summary>
    /// 按 VID/PID 查找 USB 转串口设备的 COM 口。
    /// 扫描 WMI <c>Win32_PnPEntity</c>，用正则匹配 DeviceID 中的 <c>VID_xxxx&amp;PID_xxxx</c>（不区分大小写、不依赖前导零），
    /// 返回第一个含 "(COMn)" 的 COM 口名；未找到返回 null。
    /// </summary>
    /// <param name="vid">厂商 ID（如 0x2E19）。</param>
    /// <param name="pid">产品 ID（如 0x02F9）。</param>
    /// <returns>COM 口名（如 "COM5"），未找到返回 null。</returns>
    public string? FindComPortByVidPid(int vid, int pid)
    {
        try
        {
            // 用正则匹配，兼容前导零差异：VID_2E19、VID_02E19、PID_2F9、PID_02F9 等均匹配
            var pattern = $@"VID_0*{vid:X}&.*PID_0*{pid:X}";
    ...
    /// <summary>
    /// 按 VID 查找 USB 转串口设备的 COM 口（忽略 PID）。
    /// 适用于 PID 不确定或设备使用复合接口（MI_xx）的场景。
    /// </summary>
    /// <param name="vid">厂商 ID（如 0x2E19）。</param>
    /// <returns>COM 口名列表。</returns>
    public IReadOnlyList<string> FindComPortsByVid(int vid)
    {
        var result = new List<string>();
        try
        {
            var pattern = $@"VID_0*{vid:X}[&\\]";
            var rx = new Regex(pattern, RegexOptions.IgnoreCase | RegexOptions.Compiled);
            using var searcher = new ManagementObjectSearcher(
                "SELECT Caption, DeviceID FROM Win32_PnPEntity WHERE Caption LIKE '%(COM%'");
            foreach (ManagementObject mo in searcher.Get())
            {
                ...
                if (rx.IsMatch(deviceId))
                {
                    ...
                    if (m.Success)
                    {
                        result.Add("COM" + m.Groups[1].Value);
                    }
                }
            }
        }
