...
namespace SYST.Updater;

/// <summary>
/// SYST 升级器：独立于主程序运行（升级时被拷到 %TEMP% 执行），负责
/// 等主进程退出 → 解压新版 → 回填保留项 → 整目录换名（近似原子）→ 启动新版并自检 → 失败自动回滚。
/// 也承担手动回滚（--rollback，与上一版本目录互换）。
/// </summary>
/// <remarks>
/// 用法：
///   升级：SYST.Updater.exe --pid 1234 --app "D:\SYST\app" --zip "D:\SYST\app\updates\x.zip" --exe SYST.App.exe
///   回滚：SYST.Updater.exe --pid 1234 --app "D:\SYST\app" --rollback --exe SYST.App.exe
/// 目录约定（与 app 同级）：
...
internal static class Program
{
    ...
    /// <summary>
    /// Inno 卸载记录注册表子键（AppId 与 installer\SYST.iss 的 AppId 严格一致，改一处必须改另一处）。
    /// </summary>
    ...
    private static int Main(string[] args)
    {
        ...
        if (opts == null || string.IsNullOrEmpty(opts.AppDir) || string.IsNullOrEmpty(opts.ExeName))
        {
            MessageBox.Show(
                ...
                "SYST 升级器", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return 2;
        }
